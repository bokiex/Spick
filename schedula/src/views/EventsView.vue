...
<div v-if = "!loading">
    ...
    <div v-else-if="problem === null">
        ...
        <div class="flex flex-wrap justify-around gap-4 items-center p-4">
            <Card class="pt-6 hover:bg-accent max-w-64" v-for="event in events" :key="event.event_id"
                @click="navigate(event?.event_id)">
                <div class="space-y-2">
                    <img :src="getImageUrl(event?.image)" alt="" class="w-[200px] h-[150px]" />
                    <div class="flex flex-col gap-y-1.5">
                        <h1>{{ event?.event_name }}</h1>
                        <div v-if="event.time_out" class="rounded-lg font-bold w-1/2 bg-green-300 text-center">
                            RSVP open
                        </div>
                        <div v-else-if="!event.time_out" class="rounded-lg font-bold w-1/2 bg-destructive text-center">
                            RSVP closed
                        </div>
                        <div v-if="event.user_id == userID"
                            class="rounded-lg font-bold w-1/2 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-center">
                            Host
                        </div>
                        <div v-else-if="event.user_id != userID"
